...
function ResetPasswordForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      const session = await auth.resetPassword(token, password);
      saveSession(session);
      router.push("/chat");
    } catch (err) {
  ...
  if (!token) {
    return (
      <div className="space-y-4">
        <div className="text-sm text-brand-600 bg-brand-50 border border-brand-100 dark:bg-brand-600/10 dark:border-brand-600/30 dark:text-brand-200 rounded-md px-3 py-3">
