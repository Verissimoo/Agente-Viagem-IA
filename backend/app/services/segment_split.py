"""Segment split — breaks routes through GRU (São Paulo) to find cheaper combinations.

Strategy:
  - Brazil → international: GRU → destination
  - International → Brazil: origin → GRU
  - Domestic: origin → GRU + GRU → destination
  - Routes already touching GRU or wholly outside Brazil: skipped

Reuses the Kayak provider as a black box for individual leg searches.
"""
...
class SegmentSplitAgent:
    ...
    # ── Encaixe do voo doméstico na perna internacional ─────────
