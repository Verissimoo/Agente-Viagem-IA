...
class BaseBuscaMilhasAdapter(BaseSearchAdapter):
    ...
    def search(self, request: SearchRequest, use_fixtures: bool = False, debug_dump: bool = False) -> List[UnifiedOffer]:
        ...
        # Threshold de duração absurda: para qualquer rota real, voo único
        # com mais de 18h é overnight virtual no hub (BuscaMilhas devolve
        # combinações teóricas de partner award que ninguém vende). Ex.:
        # LIS-MAD via Amsterdam com 22h50min de duração — Google Flights
        # nunca mostra essas opções.
        MAX_ITINERARY_DURATION_MIN = 18 * 60
        ...
        def build_itinerary(row):
            ...
            # Drop ofertas com duração absurdamente longa — partner award lixo.
            if dur_min > MAX_ITINERARY_DURATION_MIN:
                return None
