...
def _iata_from_flight_number(numero: Any) -> str:
    """Extrai o código IATA do operating carrier a partir do número de voo
    formatado pelo BuscaMilhas. Formatos vistos: 'TP-1010', 'G31450',
    'LA 8038', 'AA-930'. Retorna '' quando não consegue identificar.

    Isso é necessário porque o BuscaMilhas devolve `companhia` como o
    PROGRAMA de milhas (ex.: "GOL" / Smiles) mesmo quando o voo é operado
    por parceiro (ex.: TAP Lisboa→Madrid emitido via Smiles). Sem isso,
    o card mostra "GOL" como operadora e o vendedor não consegue validar.
    """
...
def _resolve_carrier(con: Dict[str, Any], companhia: str) -> str:
    """Resolve a cia REAL de um segmento — operating carrier em IATA puro.

    Ordem (prefere IATA limpo, importante pro frontend agrupar/validar):
      1. Prefixo IATA do NumeroVoo (ex.: 'TP-1010' → 'TP') — mais confiável.
      2. IATA dentro de parênteses no CompanhiaAparente (ex.: 'GOL (G3)' → 'G3').
      3. CompanhiaAparente upper se for sigla curta (2-3 chars).
      4. Nome do programa em UPPER (último fallback).
    """
    import re
    from_num = _iata_from_flight_number(con.get("NumeroVoo"))
    if from_num:
        return from_num
    aparente = (con.get("CompanhiaAparente") or "").strip().upper()
    if aparente:
        # Formato comum do BuscaMilhas: "GOL (G3)", "LATAM AIRLINES (LA)".
        m = re.search(r"\(([A-Z0-9]{2,3})\)", aparente)
        if m:
            return m.group(1)
        # Se já vier como sigla curta limpa.
        if 2 <= len(aparente) <= 3:
            return aparente
    return companhia.upper()
    ...
    if conexoes:
        for con in conexoes:
            ...
            segs.append(Segment(
                ...
                departure_dt=dep_dt,
                arrival_dt=arr_dt,
                carrier=_resolve_carrier(con, companhia),
        ...
        segs.append(Segment(
            ...
            departure_dt=dep_dt,
            arrival_dt=arr_dt,
            carrier=_resolve_carrier(voo, companhia),
