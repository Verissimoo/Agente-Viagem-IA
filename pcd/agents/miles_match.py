"""
Miles Match — Fase 3 da Quebra de Trecho.

Para uma combinação Kayak já selecionada (perna doméstica + perna
internacional), busca cotações em milhas direcionadas:
...
from pcd.core.conversion import miles_to_brl
...
class MilesMatchAgent:
    ...
    def match_domestic_leg(
        ...
        return MilesMatchResult(
            ...
            no_results_reason=None if options else (
                f"Sem disponibilidade em {own_program} para essa data/companhia."
            ),
            with_baggage=with_baggage,
        )
    ...
    def match_international_leg(
        ...
        return MilesMatchResult(
            ...
            notes=notes,
            no_results_reason=None if options else (
                f"Nenhum programa retornou voos da {carrier} compatíveis com a janela."
            ),
            with_baggage=with_baggage,
        )
    ...
    def _fetch_program_rows(
        ...
        if provider == "buscamilhas":
            ...
            try:
                ...
                from miles_app.buscamilhas_offer_parser import (
